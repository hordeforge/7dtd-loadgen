...
### 1. Mono / Boehm GC (priority=13.97, conf=0.8)
Native evidence (2.5% of sampled weight): [libmonobdwgc-2.0.so]
Structured layer signals: collect_a_little_hits=50222.0
C# surface:
...
### 2. Lock / futex contention (priority=0.99, conf=0.49)
Native evidence (2.0% of sampled weight): __pthread_cond_wait_common, ___pthread_cond_timedwait64, ___pthread_cond_wait, ___pthread_mutex_unlock
C# surface:
...
- `[jit]` ×27002
- `[UnityPlayer.so]` ×5272
- `[perf-855510.map]` ×1151
- `[libmonobdwgc-2.0.so]` ×921
- `[libc.so.6]` ×861
- `__pthread_cond_wait_common` ×263
- `___pthread_cond_timedwait64` ×187
- `[steamclient.so]` ×179
- `internal_syscall_cancel` ×93
- `___pthread_cond_wait` ×80
- `___pthread_mutex_unlock` ×77
- `GC_suspend_handler` ×75
- `__pthread_mutex_unlock_usercnt` ×74
- `__pthread_cleanup_pop` ×73
- `lll_mutex_unlock_optimized` ×69
- `mono_thread_info_uninstall_interrupt` ×60
- `GC_dirty_inner` ×54
- `syscall_cancel` ×53
- `mono_threads_exit_gc_safe_region_internal` ×52
- `__condvar_confirm_wakeup` ×46
